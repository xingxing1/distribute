...
from models.database import Admin, Student, Project
from models.exts import db
import config, os
from methods import get_Info, to_Data, to_List, to_Json, new_avatar_name, create_xlsx
from flask_cors import *
import time
from models.myRedis import myRedis
from blueprint import produce_blue
from blueprint.produce import api

app = Flask(__name__)
CORS(app, supports_credentials=True)  # 解决跨域问题
app.register_blueprint(produce_blue, prefix='/shop')
api.init_app(app)
...
def login():
    ...
    if account and password:
        ...
        keyRedis = myRedis.generate_redis_key(Admin.__tablename__, Admin.Adminaccount.key, account, password)
        password = myRedis.redis_conn.get(keyRedis)
        bloomRes = myRedis.bloomFilter.exists(keyRedis)
        if not bloomRes:
            return ("没有该用户: {}".format(account))
        if not password:
            admin = Admin.query.filter(Admin.Adminaccount == account).first()
            if admin:
                myRedis.bloomFilter.bfAdd("myRedis", keyRedis)
                myRedis.redis_conn.set(keyRedis, admin.Password)
                password = myRedis.redis_conn.get(keyRedis)
        print("耗时: ", time.time() - start_time)
        if not password:
            return ("没有该用户: {}".format(account))
    return ("查询到该用户信息: {}:{}".format(account, password))
...
def add_change_info():
    ...
    if sno and sname and grade and group:
        if state == 0:  # 修改
            ...
            student = Student.query.filter(Student.SNo == sno).first()
            if not student:
                student = Student(SNo=sno, Avatar=avatar, SName=sname, Grade=grade, Group=group, Telephone=telephone,
                                  WeChat=wechat,
                                  QQ=qq, MailBox=mailbox, Other=other, Occupation=occupation, WorkAddress=workaddress,
                                  Direction=direction)
...
def excel():
    ...
    if excel_path:
        ...
        response.headers["Content-Disposition"] = "attachment; filename = {}".format(
            '新思路成员信息一览表.xls'.encode().decode('latin-1'))
